...
public static class TypeExtensions
{
    ...
    /// <returns></returns>
    public static bool IsGenBy(this Type type, Type baseType)
    {
    ...
    /// <param name="type">当前类型</param>
    /// <returns></returns>
    public static bool IsGenBy<TBaseType>(this Type type)
    {
        Type baseType = typeof(TBaseType);
        return type.IsGenBy(baseType);
    }
    /// <summary>
    /// 判断当前类型是否可由指定类型派生
    /// </summary>
    ...
    {
        return type.IsClass && (canAbstract || !type.IsAbstract) && type.IsGenBy(baseType);
    }
    ...
    /// 获取类中的属性值
    /// 
    public static string GetModelValue(this Type type,string FieldName,Type types=null)
    {
        try
        {
            object o = type.GetProperty(FieldName).GetValue(types, null);
